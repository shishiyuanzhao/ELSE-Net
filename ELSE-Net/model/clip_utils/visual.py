...
class ClipOutput(dict):
    ...
    def __add__(self, other):
        result = ClipOutput(self.spacial_shape)
        for key in self.keys():
            result[key] = self[key] + other[key]  # Assuming addition is defined for the values
        return result
...
class FeatureExtractor_No(nn.Module):
    def __init__(
        self,
        visual_encoder: VisionTransformer,
        last_layer_idx: int = -1,
        ...
        self.patchnorm_pre_ln = visual_encoder.patchnorm_pre_ln
        self.conv1 = visual_encoder.conv1
        self.adapter_layer_1 = nn.Sequential(
                    nn.Linear(
                    in_features=768,
                    out_features=1024,
                    bias=False))
        ...
        self.adapter_layer_2 = nn.Linear(
                    in_features=1024,
                    out_features=768,
                    bias=False)
        # class embeddings and positional embeddings
